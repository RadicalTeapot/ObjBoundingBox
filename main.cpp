...
namespace objfiles {
	...
	double get_area( const Face &face ) {
		// Basic trigo, prefer this method to heron's formula as this method 
		// uses only 2 sqrt vs 4 for heron's formula
		// 
		// 	   third
		//	    /|\
			//	   / | \
...
// This could be improved CPU wise by caching already computed edge lengths
// to avoid un-necessary sqrts
...
// Vector representing the first side
	...
	double get_volume( const Face &face ) {
		...
		//
		// The version below is prefered for readability
